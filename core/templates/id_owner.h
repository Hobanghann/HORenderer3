...
namespace ho {
    ...
    class ID_Owner {
       public:
        ...
        template <typename U = value_type, typename = std::enable_if_t<std::is_copy_constructible_v<U>>>
        ...
        ID_type Emplace(Args&&... args) {
            base_id_.Increment();
            ID_type id = base_id_;
            id_to_val_.emplace(id, std::make_unique<value_type>(std::forward<Args>(args)...));
            return id;
        }
        ...
        bool Has(const ID_type& id) const { return !id.IsNULL() && id_to_val_.find(id) != id_to_val_.end(); }
