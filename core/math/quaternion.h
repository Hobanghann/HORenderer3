...
struct Quaternion {
 public:
  ...
  static Quaternion FromEuler(real x, real y, real z,
...
constexpr Quaternion::Quaternion(real p_x, real p_y, real p_z, real p_w)
...
constexpr real Quaternion::SqrdMagnitude() const {
...
constexpr bool Quaternion::IsUnitApprox() const {
  return math::IsEqualApprox(SqrdMagnitude(), 1.0_r);
}
