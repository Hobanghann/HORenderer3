...
struct Projection {
  ...
  _ALWAYS_INLINE_ Projection(real p_fov, real p_aspect_ratio, real p_near,
                             real p_far);
  _ALWAYS_INLINE_ Projection(real p_fov, real p_width, real p_height,
...
Projection::Projection(real p_fov, real p_aspect_ratio, real p_near, real p_far)
  ...
  matrix = Matrix4x4({fov_factor / aspect_ratio, 0.0_r, 0.0_r, 0.0_r},
                     {0.0_r, fov_factor, 0.0_r, 0.0_r},
                     {0.0_r, 0.0_r, -(near + far) * inv_near_minus_far,
                      -(2.0_r * near * far) * inv_near_minus_far},
                     {0.0_r, 0.0_r, -1.0_r, 0.0_r});
}
Projection::Projection(real p_fov, real p_width, real p_height, real p_near,
                       real p_far)
...
constexpr bool Projection::operator==(const Projection& rhs) const {
  return matrix == rhs.matrix && fov == rhs.fov &&
         aspect_ratio == rhs.aspect_ratio && near == rhs.near && far == rhs.far;
}
constexpr bool Projection::operator!=(const Projection& rhs) const {
  return !(*this == rhs);
}
...
constexpr bool Projection::IsEqualApprox(const Projection& rhs) const {
  return matrix.IsEqualApprox(rhs.matrix) &&
         math::IsEqualApprox(fov, rhs.fov) &&
         math::IsEqualApprox(aspect_ratio, rhs.aspect_ratio) &&
         math::IsEqualApprox(near, rhs.near) &&
...
_ALWAYS_INLINE_ constexpr Matrix4x4 operator*(const Projection& p,
                                              const Transform3D& t) {
