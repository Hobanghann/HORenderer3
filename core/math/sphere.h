...
namespace ho {
    ...
    constexpr Sphere::Sphere(const Vector3& p_center, real p_radius) : center(p_center), radius(p_radius) {}

    constexpr Sphere& Sphere::operator=(const Sphere& bounding_volume) = default;

    constexpr bool Sphere::operator==(const Sphere& rhs) const { return center == rhs.center && radius == rhs.radius; }
    constexpr bool Sphere::operator!=(const Sphere& rhs) const { return !(*this == rhs); }
    ...
    constexpr bool Sphere::IsNotEqualApprox(const Sphere& rhs) const { return !IsEqualApprox(rhs); }
}  // namespace ho
