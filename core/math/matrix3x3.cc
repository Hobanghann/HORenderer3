...
Matrix3x3 Matrix3x3::FromEuler(real x, real y, real z, math::EulerOrder order) {
  ...
  math::SinCos(sx, cx, x);
  const Matrix3x3 mx =
      Matrix3x3({1.0_r, 0.0_r, 0.0_r}, {0.0_r, cx, -sx}, {0.0_r, sx, cx});
  real sy, cy;
  math::SinCos(sy, cy, y);
  const Matrix3x3 my =
      Matrix3x3({cy, 0.0_r, sy}, {0.0_r, 1.0_r, 0.0_r}, {-sy, 0.0_r, cy});
  real sz, cz;
  math::SinCos(sz, cz, z);
  const Matrix3x3 mz = {
      {cz, -sz, 0.0_r}, {sz, cz, 0.0_r}, {0.0_r, 0.0_r, 1.0_r}};
...
std::string Matrix3x3::ToString() const {
  ...
  std::snprintf(
      res, sizeof(res),
      "| %.3f , %.3f , %.3f |\n| %.3f , %.3f , %.3f |\n| %.3f , %.3f , %.3f |",
      (float)row0.x, (float)row0.y, (float)row0.z, (float)row1.x, (float)row1.y,
      (float)row1.z, (float)row2.x, (float)row2.y, (float)row2.z);
