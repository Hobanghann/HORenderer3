...
namespace ho {
    struct Basis3D {
        ...
        static Basis3D FromEuler(real x, real y, real z, math::EulerOrder order = math::YXZ);

        constexpr Basis3D();
        constexpr Basis3D(const Basis3D& transform) = default;
        constexpr Basis3D(const Vector3& basis_x, const Vector3& basis_y, const Vector3& basis_z);
        constexpr Basis3D(real scale_x, real scale_y, real scale_z, const Vector3& basis_x, const Vector3& basis_y,
        ...
        Basis3D& RotateEuler(real x_angle, real y_angle, real z_angle, math::EulerOrder order = math::YXZ);
        Basis3D& RotateAxisAngle(const Vector3& axis, real angle);
        constexpr Basis3D& RotateQuaternion(const Quaternion& q);
        Basis3D RotatedEuler(real x_angle, real y_angle, real z_angle, math::EulerOrder order = math::YXZ) const;
        Basis3D RotatedAxisAngle(const Vector3& axis, real angle) const;
        constexpr Basis3D RotatedQuaternion(const Quaternion& q) const;

        Basis3D& RotateEulerLocal(real x_angle, real y_angle, real z_angle, math::EulerOrder order = math::YXZ);
        Basis3D& RotateAxisAngleLocal(const Vector3& axis, real angle);
        constexpr Basis3D& RotateQuaternionLocal(const Quaternion& q);
        Basis3D RotatedEulerLocal(real x_angle, real y_angle, real z_angle, math::EulerOrder order = math::YXZ) const;
        ...
        Basis3D LookedAt(const Vector3& at, const Vector3& up, bool is_facing = false) const;
    ...
    constexpr Basis3D::Basis3D() : row0(Vector3::UNIT_X), row1(Vector3::UNIT_Y), row2(Vector3::UNIT_Z) {}
    constexpr Basis3D::Basis3D(const Vector3& basis_x, const Vector3& basis_y, const Vector3& basis_z)
        : row0(basis_x.x, basis_y.x, basis_z.x),
          row1(basis_x.y, basis_y.y, basis_z.y),
          row2(basis_x.z, basis_y.z, basis_z.z) {}
    constexpr Basis3D::Basis3D(real scale_x, real scale_y, real scale_z, const Vector3& basis_x, const Vector3& basis_y,
                               const Vector3& basis_z)
    ...
    constexpr Basis3D::Basis3D(const Quaternion& q) : matrix(Matrix3x3::FromQuaternion(q)) {}
    ...
    constexpr Vector3 Basis3D::basis_x() const { return Vector3(row0.x, row1.x, row2.x); }
    constexpr Vector3 Basis3D::basis_y() const { return Vector3(row0.y, row1.y, row2.y); }
    constexpr Vector3 Basis3D::basis_z() const { return Vector3(row0.z, row1.z, row2.z); }
    ...
    constexpr bool Basis3D::operator==(const Basis3D& rhs) const { return matrix == rhs.matrix; }
    constexpr bool Basis3D::operator!=(const Basis3D& rhs) const { return !(*this == rhs); }

    constexpr bool Basis3D::IsEqualApprox(const Basis3D& rhs) const { return matrix.IsEqualApprox(rhs.matrix); }
    constexpr bool Basis3D::IsNotEqualApprox(const Basis3D& rhs) const { return !IsEqualApprox(rhs); }
    ...
    constexpr Vector3 Basis3D::Transform(const Vector3& v) const { return matrix * v; }
    constexpr Vector3 Basis3D::InvTransform(const Vector3& v) const { return Inverse().Transform(v); }
    constexpr Vector3 Basis3D::InvTransformFast(const Vector3& v) const { return InverseFast().Transform(v); }
}  // namespace ho
