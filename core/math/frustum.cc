...
Frustum Frustum::FromMatrix4x4(const Matrix4x4& m) {
  ...
  Plane left =
      Plane(coefficients.x, coefficients.y, coefficients.z, coefficients.w);
...
math::Side Frustum::GetSide(const Vector3& p1, const Vector3& p2,
...
std::string Frustum::ToString() const {
  ...
  snprintf(buf, sizeof(buf),
           "Left: %s\nRight: %s\nTop: %s\nBottom: %s\nNear: %s\nFar: %s",
           planes[PLANE_POS_LEFT].ToString().c_str(),
           planes[PLANE_POS_RIGHT].ToString().c_str(),
           planes[PLANE_POS_TOP].ToString().c_str(),
           planes[PLANE_POS_BOTTOM].ToString().c_str(),
           planes[PLANE_POS_NEAR].ToString().c_str(),
           planes[PLANE_POS_FAR].ToString().c_str());
  return buf;
}
}  // namespace ho