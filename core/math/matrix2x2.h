...
constexpr Matrix2x2::Matrix2x2(const Vector2& p_row0, const Vector2& p_row1)
    : row0(p_row0), row1(p_row1) {}

_INLINE_ constexpr Matrix2x2 Matrix2x2::IDENTITY =
    Matrix2x2(Vector2(1.0_r, 0.0_r), Vector2(0.0_r, 1.0_r));
...
constexpr Matrix2x2 Matrix2x2::operator*(const Matrix2x2& rhs) const {
  ...
  return Matrix2x2(Vector2(row0.Dot(rhs_c1), row0.Dot(rhs_c2)),
                   Vector2(row1.Dot(rhs_c1), row1.Dot(rhs_c2)));
}
constexpr Matrix2x2& Matrix2x2::operator*=(const Matrix2x2& rhs) {
  const Vector2 rhs_c1 = rhs.col0();
  const Vector2 rhs_c2 = rhs.col1();
...
constexpr Matrix2x2 Matrix2x2::operator*(real scalar) const {
  return Matrix2x2(scalar * row0, scalar * row1);
}
...
constexpr Matrix2x2 Matrix2x2::operator/(real scalar) const {
  return Matrix2x2(row0 / scalar, row1 / scalar);
}
constexpr Matrix2x2& Matrix2x2::operator/=(real scalar) {
  row0 /= scalar;
  row1 /= scalar;
...
constexpr real Matrix2x2::Determinant() const {
  return row0.x * row1.y - row0.y * row1.x;
}

_ALWAYS_INLINE_ constexpr Matrix2x2 operator*(float scalar,
                                              const Matrix2x2& matrix) {
  return Matrix2x2(scalar * matrix.row0, scalar * matrix.row1);
}

_ALWAYS_INLINE_ constexpr Vector2 operator*(const Matrix2x2& m,
                                            const Vector2& v) {
