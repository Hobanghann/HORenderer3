...
struct Matrix3x3 {
  ...
  static Matrix3x3 FromEuler(real x, real y, real z,
                             math::EulerOrder order = math::YXZ);
  constexpr static Matrix3x3 FromQuaternion(const Quaternion& q);

  constexpr Matrix3x3();
  constexpr Matrix3x3(const Vector3& p_row0, const Vector3& p_row1,
                      const Vector3& p_row2);
...
constexpr Matrix3x3::Matrix3x3()
    : row0(Vector3()), row1(Vector3()), row2(Vector3()) {}

constexpr Matrix3x3::Matrix3x3(const Vector3& p_row0, const Vector3& p_row1,
                               const Vector3& p_row2)
...
constexpr Vector3 Matrix3x3::col0() const {
  return Vector3(row0.x, row1.x, row2.x);
}
constexpr Vector3 Matrix3x3::col1() const {
  return Vector3(row0.y, row1.y, row2.y);
}
constexpr Vector3 Matrix3x3::col2() const {
...
constexpr Matrix3x3 Matrix3x3::operator*(const Matrix3x3& rhs) const {
  ...
  return Matrix3x3(
      Vector3(row0.Dot(rhs_c1), row0.Dot(rhs_c2), row0.Dot(rhs_c3)),
...
constexpr Matrix3x3 Matrix3x3::Transposed() const {
  return Matrix3x3(Vector3(row0.x, row1.x, row2.x),
...
_ALWAYS_INLINE_ constexpr Matrix3x3 operator*(real scalar,
                                              const Matrix3x3& matrix) {
  return Matrix3x3(scalar * matrix.row0, scalar * matrix.row1,
                   scalar * matrix.row2);
}

_ALWAYS_INLINE_ constexpr Vector3 operator*(const Matrix3x3& m,
                                            const Vector3& v) {
