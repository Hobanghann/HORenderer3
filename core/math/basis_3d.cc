...
Basis3D& Basis3D::RotateEuler(real x_angle, real y_angle, real z_angle,
...
Basis3D Basis3D::RotatedEuler(real x_angle, real y_angle, real z_angle,
...
Basis3D& Basis3D::RotateEulerLocal(real x_angle, real y_angle, real z_angle,
                                   math::EulerOrder order) {
  const Matrix3x3 rm = Matrix3x3::FromEuler(x_angle, y_angle, z_angle, order);
  matrix = matrix * rm;
  return *this;
...
Basis3D Basis3D::RotatedEulerLocal(real x_angle, real y_angle, real z_angle,
                                   math::EulerOrder order) const {
  Basis3D b = *this;
...
Basis3D& Basis3D::LookAt(const Vector3& at, const Vector3& up,
...
Basis3D Basis3D::LookedAt(const Vector3& target, const Vector3& up,
...
std::string Basis3D::ToString() const {
  char buf[200];
  snprintf(buf, sizeof(buf), "Basis3D X: %s, Y: %s, Z: %s",
           basis_x().ToString().c_str(), basis_y().ToString().c_str(),
           basis_z().ToString().c_str());
  return buf;
}
}  // namespace ho