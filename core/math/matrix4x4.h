...
struct Matrix4x4 {
  ...
  constexpr Matrix4x4();
  constexpr Matrix4x4(const Vector4& p_row0, const Vector4& p_row1,
...
constexpr Matrix4x4::Matrix4x4()
    : row0(Vector4()), row1(Vector4()), row2(Vector4()), row3(Vector4()) {}

constexpr Matrix4x4::Matrix4x4(const Vector4& p_row0, const Vector4& p_row1,
                               const Vector4& p_row2, const Vector4& p_row3)
    : row0(p_row0), row1(p_row1), row2(p_row2), row3(p_row3) {}

_INLINE_ constexpr Matrix4x4 Matrix4x4::IDENTITY = Matrix4x4(
    Vector4(1.0_r, 0.0_r, 0.0_r, 0.0_r), Vector4(0.0_r, 1.0_r, 0.0_r, 0.0_r),
...
constexpr Vector4 Matrix4x4::col0() const {
  return Vector4(row0.x, row1.x, row2.x, row3.x);
}
constexpr Vector4 Matrix4x4::col1() const {
  return Vector4(row0.y, row1.y, row2.y, row3.y);
}
constexpr Vector4 Matrix4x4::col2() const {
  return Vector4(row0.z, row1.z, row2.z, row3.z);
}
constexpr Vector4 Matrix4x4::col3() const {
...
constexpr Matrix4x4 Matrix4x4::operator+(const Matrix4x4& rhs) const {
  return Matrix4x4(row0 + rhs.row0, row1 + rhs.row1, row2 + rhs.row2,
                   row3 + rhs.row3);
}
constexpr Matrix4x4& Matrix4x4::operator+=(const Matrix4x4& rhs) {
  row0 += rhs.row0;
...
constexpr Matrix4x4 Matrix4x4::operator*(const Matrix4x4& rhs) const {
  const Vector4 c1 = rhs.col0(), c2 = rhs.col1(), c3 = rhs.col2(),
                c4 = rhs.col3();
  return Matrix4x4(
      Vector4(row0.Dot(c1), row0.Dot(c2), row0.Dot(c3), row0.Dot(c4)),
...
constexpr bool Matrix4x4::IsEqualApprox(const Matrix4x4& r) const {
  return row0.IsEqualApprox(r.row0) && row1.IsEqualApprox(r.row1) &&
         row2.IsEqualApprox(r.row2) && row3.IsEqualApprox(r.row3);
}
...
constexpr bool Matrix4x4::IsOrthonormal() const {
  return IsOrthogonal() && col0().IsUnitApprox() && col1().IsUnitApprox() &&
         col2().IsUnitApprox() && col3().IsUnitApprox();
}

constexpr Matrix4x4& Matrix4x4::Transpose() {
  *this = Matrix4x4(Vector4(row0.x, row1.x, row2.x, row3.x),
...
constexpr Matrix4x4& Matrix4x4::Invert() {
  ...
  if (math::IsZeroApprox(det)) {
    const Vector4 nanv(math::REAL_NaN, math::REAL_NaN, math::REAL_NaN,
                       math::REAL_NaN);
...
constexpr real Matrix4x4::Trace() const {
...
constexpr real Matrix4x4::Determinant() const {
  ...
  return row0.x * (row1.y * subfactor00 - row1.z * subfactor01 +
                   row1.w * subfactor02) -
         row0.y * (row1.x * subfactor00 - row1.z * subfactor03 +
                   row1.w * subfactor04) +
         row0.z * (row1.x * subfactor01 - row1.y * subfactor03 +
                   row1.w * subfactor05) -
         row0.w * (row1.x * subfactor02 - row1.y * subfactor04 +
                   row1.z * subfactor05);
}
...
_ALWAYS_INLINE_ constexpr Vector4 operator*(const Matrix4x4& m,
                                            const Vector4& v) {
