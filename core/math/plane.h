...
struct Plane {
  ...
  _ALWAYS_INLINE_ math::Side GetSide(const Vector3& p1,
                                     const Vector3& p2) const;
  _ALWAYS_INLINE_ math::Side GetSide(const Vector3& p1, const Vector3& p2,
 ...
 private:
  template <size_t N>
  _ALWAYS_INLINE_ math::Side GetSide(
      const std::array<Vector3, N>& points) const;
};
...
real Plane::DistToPoint(const Vector3& point) const {
...
math::Side Plane::GetSide(const Vector3& p1, const Vector3& p2,
                          const Vector3& p3) const {
  int num_outside = 0;
  if (GetSide(p1) == math::OUTSIDE) {
    num_outside++;
