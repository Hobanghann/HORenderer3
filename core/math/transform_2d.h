...
constexpr Transform2D::Transform2D()
    : basis(Basis2D()), origin(Vector2::ZERO) {}
constexpr Transform2D::Transform2D(const Basis2D& p_basis,
                                   const Vector2& p_origin)
    : basis(p_basis), origin(p_origin) {}
constexpr Transform2D::Transform2D(const Matrix3x3& m)
    : basis(Basis2D(Vector2(m.data[0][0], m.data[1][0]),
                    Vector2(m.data[0][1], m.data[1][1]))),
...
constexpr void Transform2D::set_basis_x(const Vector2& basis_x) {
  basis.set_basis_x(basis_x);
}
constexpr void Transform2D::set_basis_y(const Vector2& basis_y) {
...
constexpr Transform2D Transform2D::Translated(
    const Vector2& translation_v) const {
  Transform2D copy = *this;
...
constexpr Transform2D& Transform2D::TranslateLocal(
    const Vector2& translation_v) {
...
constexpr Transform2D Transform2D::TranslatedLocal(
    const Vector2& translation_v) const {
  Transform2D copy = *this;
...
constexpr Matrix3x3 Transform2D::ToMatrix() const {
  return Matrix3x3(Vector3(basis.row0.x, basis.row0.y, origin.x),
                   Vector3(basis.row1.x, basis.row1.y, origin.y),
