...
class FrustumTest : public ::testing::Test {
 protected:
  void SetUp() override {
    p_default = Plane();
    left =
        Plane(Vector3(-1.0_r, 0.0_r, 0.0_r), Vector3(-1.0_r, 0.0_r, 0.0_r));
    right =
        Plane(Vector3(1.0_r, 0.0_r, 0.0_r), Vector3(1.0_r, 0.0_r, 0.0_r));
    bottom =
        Plane(Vector3(0.0_r, -1.0_r, 0.0_r), Vector3(0.0_r, -1.0_r, 0.0_r));
    top =
        Plane(Vector3(0.0_r, 1.0_r, 0.0_r), Vector3(0.0_r, 1.0_r, 0.0_r));
    near =
        Plane(Vector3(0.0_r, 0.0_r, -1.0_r), Vector3(0.0_r, 0.0_r, -1.0_r));
    far =
        Plane(Vector3(0.0_r, 0.0_r, 1.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));
...
TEST_F(FrustumTest, Assignment) {
  ...
  for (int i = 0; i < 6; ++i) {
    EXPECT_TRUE(f_default.planes[static_cast<PlanePos>(i)].IsEqualApprox(
        f1.planes[static_cast<PlanePos>(i)]));
  }
...
TEST_F(FrustumTest, GetSideTwoPoints) {
  ...
  EXPECT_EQ(f1.GetSide(outside_posZ, Vector3(0.0_r, 0.0_r, 3.0_r)),
            math::OUTSIDE);
  EXPECT_EQ(f1.GetSide(outside_posX, Vector3(3.0_r, 0.0_r, 0.0_r)),
            math::OUTSIDE);
}
...
TEST_F(FrustumTest, FromMatrix4x4) {
  ...
  EXPECT_FALSE(
      f.planes[PLANE_POS_LEFT].IsEqualApprox(f.planes[PLANE_POS_RIGHT]));
  EXPECT_FALSE(
      f.planes[PLANE_POS_TOP].IsEqualApprox(f.planes[PLANE_POS_BOTTOM]));
