...
class Transform2DTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        t_non_ortho = Transform2D(Basis2D(Vector2(1.0_r, 0.2_r), Vector2(0.3_r, 1.0_r)), Vector2(2.0_r, 1.0_r));
    }
...
TEST_F(Transform2DTest, ApproximateEqualityCheck) {
    Transform2D t1(b_scale, Vector2(1.0_r, 2.0_r));
    Transform2D t2(Basis2D(Vector2(2.000001_r, 0.0_r), Vector2(0.0_r, 3.000001_r)), Vector2(1.000001_r, 2.000001_r));
...
TEST_F(Transform2DTest, Scaling) {
    ...
    Matrix3x3 m_scale(Vector3(2.0_r, 0.0_r, 0.0_r), Vector3(0.0_r, 3.0_r, 0.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));
    ...
    Matrix3x3 m_uniform(Vector3(2.0_r, 0.0_r, 0.0_r), Vector3(0.0_r, 2.0_r, 0.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));
...
TEST_F(Transform2DTest, Rotation) {
    ...
    real angle = PI / 4.0_r;
    real s = std::sin(angle), c = std::cos(angle);
    Matrix3x3 m_rot(Vector3(c, -s, 0.0_r), Vector3(s, c, 0.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));

    Transform2D t1 = t;
    t1.Rotate(angle);
    EXPECT_TRUE(t1.ToMatrix().IsEqualApprox(m_rot * m_base)) << t1.ToMatrix().ToString() << "\n"
...
TEST_F(Transform2DTest, Translation) {
    ...
    Matrix3x3 m_trans(Vector3(1.0_r, 0.0_r, trans.x), Vector3(0.0_r, 1.0_r, trans.y), Vector3(0.0_r, 0.0_r, 1.0_r));
...
TEST_F(Transform2DTest, InversionFast) {
    ...
    t1 = t_rot;
    t1.InvertFast();
    EXPECT_TRUE(t1.ToMatrix().IsEqualApprox(t_rot.ToMatrix().Inverse())) << t1.ToMatrix().ToString() << "\n"
...
TEST_F(Transform2DTest, TransformAndInvTransform) {
    ...
    EXPECT_TRUE(t.Transform(v).IsEqualApprox(Vector2(m * Vector3(v.x, v.y, 1.0_r))));
    EXPECT_TRUE(t.InvTransform(v).IsEqualApprox(Vector2(m.Inverse() * Vector3(v.x, v.y, 1.0_r))));
    EXPECT_FALSE(t.InvTransformFast(v).IsEqualApprox(Vector2(m.Inverse() * Vector3(v.x, v.y, 1.0_r))));

    Transform2D t_ortho(b_rot, Vector2(1.0_r, 2.0_r));
    EXPECT_TRUE(
        t_ortho.InvTransformFast(v).IsEqualApprox(Vector2(t_ortho.ToMatrix().Inverse() * Vector3(v.x, v.y, 1.0_r))));
}
