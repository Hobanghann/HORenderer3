...
TEST(Color128Test, Division) {
  Color128 div = Color128(0.5f, 0.6f, 0.8f, 1.0f) /
                 Color128(0.5f, 0.2f, 0.4f, 0.0f);
...
TEST(Color128Test, Luminance) {
  ...
  EXPECT_NEAR(lum_custom, 0.3f * 0.4f + 0.59f * 0.5f + 0.11f * 0.6f,
              math::EPSILON_CMP);
}
...
TEST(Color128Test, ToVector3Conversion) {
  ...
  Color128 c2((v.x + 1.0f) / 2.0f, (v.y + 1.0f) / 2.0f,
              (v.z + 1.0f) / 2.0f);
