...
TEST(InterpFuncsTest, PcerpWithInvW2DNormalCase) {
    ...
    Vector2 inv_w(1.0_r, 0.5_r);  // w1=1, w2=2
    Vector2 bary(0.25_r, 0.75_r);
    real interpolated_w = Pcerp_W(inv_w, bary);  // = 1 / (0.25*1 + 0.75*0.5) = 1.6
...
TEST(InterpFuncsTest, QuaternionSlerpNormalCase) {
    ...
    Quaternion expected = Quaternion::FromAxisAngle(Vector3::UNIT_Y, math::PI * 0.25_r);
...
TEST(InterpFuncsTest, QuaternionSlerpLongNormalCase) {
    ...
    Quaternion expected = Quaternion::FromAxisAngle(Vector3::UNIT_Y, math::PI + math::PI * 0.25_r);
