...
TEST_F(Basis2DTest, LookAt) {
  ...
  EXPECT_TRUE(
      b.IsEqualApprox(Basis2D(Vector2(0.0_r, -2.0_r), Vector2(3.0_r, 0.0_r))));
}
...
TEST_F(Basis2DTest, Multiplication) {
  ...
  Matrix2x2 m1 =
      Matrix2x2(Vector2(1.0_r, 2.0_r), Vector2(3.0_r, 4.0_r)).Transposed();
  Matrix2x2 m2 =
      Matrix2x2(Vector2(4.0_r, 3.0_r), Vector2(2.0_r, 1.0_r)).Transposed();
...
TEST_F(Basis2DTest, TransformAndInvTransform) {
  ...
  EXPECT_TRUE(
      b_rot.InvTransformFast(v).IsEqualApprox(b_rot.matrix.Inverse() * v));
}
