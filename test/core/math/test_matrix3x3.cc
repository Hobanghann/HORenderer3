...
class Matrix3x3Test : public ::testing::Test {
   protected:
    void SetUp() override {
        m_default = Matrix3x3();
        m_eye = Matrix3x3(Vector3(1.0_r, 0.0_r, 0.0_r), Vector3(0.0_r, 1.0_r, 0.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));

        m_19 = Matrix3x3(Vector3(1.0_r, 2.0_r, 3.0_r), Vector3(4.0_r, 5.0_r, 6.0_r), Vector3(7.0_r, 8.0_r, 9.0_r));

        m_91 = Matrix3x3(Vector3(9.0_r, 8.0_r, 7.0_r), Vector3(6.0_r, 5.0_r, 4.0_r), Vector3(3.0_r, 2.0_r, 1.0_r));

        const real angle = PI / 4.0_r;
        const real s = std::sin(angle);
        const real c = std::cos(angle);

        m_pitch = Matrix3x3(Vector3(1.0_r, 0.0_r, 0.0_r), Vector3(0.0_r, c, -s), Vector3(0.0_r, s, c));
        m_yaw = Matrix3x3(Vector3(c, 0.0_r, s), Vector3(0.0_r, 1.0_r, 0.0_r), Vector3(-s, 0.0_r, c));
        m_roll = Matrix3x3(Vector3(c, -s, 0.0_r), Vector3(s, c, 0.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));

        m_orthogonal =
            Matrix3x3(Vector3(2 * c, -3 * s, 0.0_r), Vector3(2 * s, 3 * c, 0.0_r), Vector3(0.0_r, 0.0_r, 1.0_r));
        m_non_singular =
            Matrix3x3(Vector3(1.0_r, 2.0_r, 3.0_r), Vector3(0.0_r, 2.0_r, 4.0_r), Vector3(5.0_r, 6.0_r, 0.0_r));
    }

    Matrix3x3 m_default, m_eye, m_19, m_91, m_pitch, m_yaw, m_roll, m_orthogonal, m_non_singular;
};
...
TEST_F(Matrix3x3Test, FromEuler) {
    const real angle = PI / 4.0_r;
    EXPECT_TRUE(Matrix3x3::FromEuler(angle, angle, angle, math::XYZ).IsEqualApprox(m_pitch * m_yaw * m_roll));
    EXPECT_TRUE(Matrix3x3::FromEuler(angle, angle, angle, math::XZY).IsEqualApprox(m_pitch * m_roll * m_yaw));
    EXPECT_TRUE(Matrix3x3::FromEuler(angle, angle, angle, math::YZX).IsEqualApprox(m_yaw * m_roll * m_pitch));
    EXPECT_TRUE(Matrix3x3::FromEuler(angle, angle, angle, math::YXZ).IsEqualApprox(m_yaw * m_pitch * m_roll));
    EXPECT_TRUE(Matrix3x3::FromEuler(angle, angle, angle, math::ZXY).IsEqualApprox(m_roll * m_pitch * m_yaw));
    EXPECT_TRUE(Matrix3x3::FromEuler(angle, angle, angle, math::ZYX).IsEqualApprox(m_roll * m_yaw * m_pitch));
}

TEST_F(Matrix3x3Test, FromQuaternion) {
    Quaternion q = Quaternion::FromAxisAngle(Vector3(0.0_r, 0.0_r, 1.0_r), PI / 4.0_r);
...
TEST_F(Matrix3x3Test, MatrixMultiplication) {
    Matrix3x3 expected(Vector3(30.0_r, 24.0_r, 18.0_r), Vector3(84.0_r, 69.0_r, 54.0_r),
...
TEST_F(Matrix3x3Test, ComparisonOperators) {
    Matrix3x3 a(Vector3(1.0_r, 2.0_r, 3.0_r), Vector3(4.0_r, 5.0_r, 6.0_r), Vector3(7.0_r, 8.0_r, 9.0_r));
...
TEST_F(Matrix3x3Test, ApproximateEqualityCheck) {
    Matrix3x3 near_19(Vector3(1.000001_r, 2.000001_r, 3.000001_r), Vector3(4.000001_r, 5.000001_r, 6.000001_r),
...
TEST_F(Matrix3x3Test, Inversion) {
    Matrix3x3 m(Vector3(1.0_r, 2.0_r, 3.0_r), Vector3(0.0_r, 1.0_r, 4.0_r), Vector3(5.0_r, 6.0_r, 0.0_r));
    ...
    Matrix3x3 singular(Vector3(1.0_r, 2.0_r, 3.0_r), Vector3(2.0_r, 4.0_r, 6.0_r), Vector3(3.0_r, 6.0_r, 9.0_r));
...
TEST_F(Matrix3x3Test, Trace) { EXPECT_FLOAT_EQ(m_19.Trace(), 1.0_r + 5.0_r + 9.0_r); }
