...
TEST(ResourceImporterTest, ImportModelOBJ) {
    std::string file_path = (std::filesystem::path("test_assets") / "test_obj" / "obj_cube" / "cube.obj").string();
    RunImporterTest(Path(file_path));
}

TEST(ResourceImporterTest, ImportModelGLTF) {
    std::string file_path = (std::filesystem::path("test_assets") / "test_gltf" / "Box" / "glTF" / "Box.gltf").string();
    RunImporterTest(Path(file_path));
}
