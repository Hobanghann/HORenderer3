    ...
    struct Skeleton {
        ...
        Skeleton(std::string&& p_name, std::vector<std::string>&& p_bone_names,
            ...
            for (std::size_t i = 0; i < parents.size(); ++i) {
                ...
                if (parent >= 0) {
                    children[static_cast<size_t>(parent)].push_back(static_cast<uint32_t>(i));
                }
            }
        ...
        ALWAYS_INLINE uint32_t GetBoneCount() const { return static_cast<uint32_t>(bone_names.size()); }

        ALWAYS_INLINE uint32_t GetBoneIndex(const std::string& p_name) const {
            auto it = name_to_index.find(p_name);
            return it->second;
        }
        ALWAYS_INLINE const std::string& GetBoneName(uint32_t index) const { return bone_names[index]; }
        ...
        ALWAYS_INLINE const Transform3D& GetLocalTransform(uint32_t index) const { return local_transforms[index]; }

        ALWAYS_INLINE int GetParentIndex(const std::string& p_name) const {
            return GetParentIndex(GetBoneIndex(p_name));
        }
        ALWAYS_INLINE int GetParentIndex(uint32_t index) const { return parents[index]; }
        ...
        ALWAYS_INLINE uint32_t GetChildIndex(const std::string& parent_name, uint32_t child_index) const {
            return GetChildIndex(GetBoneIndex(parent_name), child_index);
        }
        ALWAYS_INLINE uint32_t GetChildIndex(uint32_t parent_index, uint32_t child_index) const {
