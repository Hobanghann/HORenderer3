...
struct Mesh {
 public:
  ...
  struct MorphTarget {
    MorphTarget() = default;
    MorphTarget(std::string&& p_name, std::vector<Vector3>&& p_positions,
                std::vector<Vector3>&& p_normals,
  ...
  struct SubMesh {
    SubMesh() = default;
    SubMesh(
        std::string&& p_name, PrimitiveType p_primitive_type,
        std::vector<Vector3>&& p_positions, std::vector<Vector3>&& p_normals,
        std::vector<Vector4>&& p_tangents,
        ...
        std::vector<MorphTarget>&& p_morph_targets,
    ...
    ALWAYS_INLINE const MorphTarget& GetMorphTarget(
    ...
    GetMorphTargetIndex(const std::string& p_name) const {
      auto it = morph_name_to_index.find(p_name);
      assert(it != morph_name_to_index.end());
      return it->second;
    }
    ALWAYS_INLINE uint32_t GetMorphTargetCount() const {
      return (uint32_t)morph_targets.size();
    }
  ...
  Mesh(std::string&& p_name, std::vector<SubMesh>&& p_sub_meshes) noexcept
    ...
    if (!sub_meshes.empty()) {
      ...
      for (const auto& sm : sub_meshes) {
        all_positions.insert(all_positions.end(), sm.positions.begin(),
                             sm.positions.end());
      }
  ...
  ALWAYS_INLINE uint32_t GetSubMeshCount() const {
    return (uint32_t)sub_meshes.size();
  }
