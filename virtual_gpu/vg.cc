...
void vgPolygonMode(VGenum face, VGenum mode) {
  ...
  if (face == VG_FRONT_AND_BACK &&
      (mode == VG_POINT || mode == VG_LINE || mode == VG_FILL)) {
...
void vgTexParameterf(VGenum target, VGenum pname, VGfloat param) {
  ...
  if (tu.bound_texture_targets[slot] == nullptr ||
      tu.bound_texture_targets[slot]->is_deleted) {
  ...
  switch (pname) {
    case VG_TEXTURE_LOD_BIAS:
      ds.lod_bias = param;
      break;
    case VG_TEXTURE_MIN_LOD:
      ds.min_lod = param;
      break;
    case VG_TEXTURE_MAX_LOD:
      ds.max_lod = param;
      break;
    default:
      vg.state_.error_state = VG_INVALID_ENUM;
...
void vgTexParameteri(VGenum target, VGenum pname, VGint param) {
  ...
  switch (pname) {
    case VG_TEXTURE_WRAP_S:
      if (param == (VGint)VG_CLAMP_TO_EDGE ||
          param == (VGint)VG_CLAMP_TO_BORDER ||
    ...
    case VG_TEXTURE_MIN_FILTER:
      if (param >= (VGint)VG_NEAREST &&
          param <= (VGint)VG_LINEAR_MIPMAP_LINEAR) {
        ds.min_filter = param;
      } else {
        vg.state_.error_state = VG_INVALID_ENUM;
      }
    ...
    case VG_TEXTURE_MAX_LEVEL:
      ...
      break;
    case VG_TEXTURE_COMPARE_MODE:
      if (param == (VGint)VG_COMPARE_REF_TO_TEXTURE ||
          param == (VGint)VG_NONE) {
        ds.compare_mode = param;
      } else {
        vg.state_.error_state = VG_INVALID_ENUM;
      }
      break;
    case VG_TEXTURE_COMPARE_FUNC:
      if (param >= (VGint)VG_NEVER && param <= (VGint)VG_ALWAYS) {
        ds.compare_func = param;
      } else {
        vg.state_.error_state = VG_INVALID_ENUM;
      }
      break;
    case VG_TEXTURE_SWIZZLE_R:
      if ((param >= (VGint)VG_RED && param <= (VGint)VG_ALPHA) ||
          param == (VGint)VG_ONE || param == (VGint)VG_ZERO) {
        ds.swizzle_r = param;
      } else {
        vg.state_.error_state = VG_INVALID_ENUM;
      }
...
void vgTexImage1D(VGenum target, VGint level, VGint internalformat,
  ...
  VirtualGPU::TextureObject* tex =
      vg.texture_units_[vg.active_texture_unit_].bound_texture_targets[slot];
  ...
  const size_t dst_pixel_size =
      vg::GetPixelSize(internalformat, tex->component_type);
  ...
  for (int x = 0; x < width; x++) {
    vg::CopyPixel(dst, src, tex->internal_format, tex->component_type, format,
                  type);
    src += src_pixel_size;
    dst += dst_pixel_size;
  }
...
void vgBlendFunc(VGenum sfactor, VGenum dfactor) {
  ...
  auto valid_src_factor = [](VGenum f) {
    return (f == VG_ZERO) || (f == VG_ONE) ||
           (f >= VG_SRC_COLOR && f <= VG_SRC_ALPHA_SATURATE) ||
  ...
  auto valid_dst_factor = [](VGenum f) {
    ...
    return (f == VG_ZERO) || (f == VG_ONE) ||
           (f >= VG_SRC_COLOR && f <= VG_ONE_MINUS_SRC_ALPHA) ||
           (f >= VG_CONSTANT_COLOR && f <= VG_ONE_MINUS_CONSTANT_ALPHA);
  };
...
void vgStencilOp(VGenum sfail, VGenum dpfail, VGenum dppass) {
  ...
  if (!IsValidStencilOp(sfail) || !IsValidStencilOp(dpfail) ||
...
void vgDrawArrays(VGenum mode, VGint first, VGsizei count) {
  ...
  if ((size_t)(first + count) > vg.bound_vertex_array_->vertex_count) {
    vg.state_.error_state = VG_INVALID_OPERATION;
    return;
  }
  ...
  for (int i = first; i < first + count; i += BATCH_SIZE) {
    ...
    VirtualGPU::VSJobInput* input = new VirtualGPU::VSJobInput{
        (VirtualGPU::VertexShader)vg.using_program_->vertex_shader->source,
  ...
  for (uint32_t i = 0; i + v_count <= vg.using_program_->varying_buffer.size();
       i += gap) {
    ...
    VirtualGPU::AfterVSJobInput* input = new VirtualGPU::AfterVSJobInput(
        {std::move(poly), (VirtualGPU::FragmentShader)
                              vg.using_program_->fragment_shader->source});
...
void vgDrawElements(VGenum mode, VGsizei count, VGenum type,
  ...
  if (vg.bound_vertex_array_ == nullptr || vg.using_program_ == nullptr ||
      vg.bound_draw_frame_buffer_ == nullptr ||
      vg.bound_vertex_array_->element_buffer == nullptr ||
  ...
  if (indices != nullptr &&
      (reinterpret_cast<uintptr_t>(indices) + count * idx_size >
       vg.bound_vertex_array_->element_buffer->memory->size())) {
    vg.state_.error_state = VG_INVALID_VALUE;
    return;
  }

  vg.using_program_->varying_buffer.resize(
      vg.bound_vertex_array_->vertex_count);
  ...
  for (uint32_t i = 0; i < vg.bound_vertex_array_->vertex_count;
       i += BATCH_SIZE) {
    uint32_t batch_end =
        math::Min(i + BATCH_SIZE - 1,
                  (uint32_t)(vg.bound_vertex_array_->vertex_count - 1));
    VirtualGPU::VSJobInput* input = new VirtualGPU::VSJobInput{
        (VirtualGPU::VertexShader)vg.using_program_->vertex_shader->source, 0,
        i, batch_end};
  ...
  uintptr_t offset =
      indices == nullptr ? 0 : reinterpret_cast<uintptr_t>(indices);
...
void vgTexSubImage2D(VGenum target, VGint level, VGint xoffset, VGint yoffset,
  ...
  if (xoffset < 0 || yoffset < 0 || xoffset + width > tex_level.width ||
      yoffset + height > tex_level.height) {
    vg.state_.error_state = VG_INVALID_VALUE;
    return;
  }
  ...
  for (int y = 0; y < height; y++) {
    uint8_t* dst = tex_level.memory->data() +
                   ((yoffset + y) * tex_level.width + xoffset) * dst_pixel_size;
    for (int x = 0; x < width; x++) {
      vg::CopyPixel(dst, src, tex->internal_format, tex->component_type, format,
                    type);
      src += src_pixel_size;
...
void vgBindTexture(VGenum target, VGuint texture) {
  ...
  // unbind
  if (texture == 0) {
    if (vg.texture_units_[vg.active_texture_unit_]
            .bound_texture_targets[slot]) {
      ReleaseTextureObject(vg.texture_units_[vg.active_texture_unit_]
                               .bound_texture_targets[slot]
                               ->id);
      vg.texture_units_[vg.active_texture_unit_].bound_texture_targets[slot] =
          nullptr;
...
void vgTexImage3D(VGenum target, VGint level, VGint internalformat,
  ...
  for (int z = 0; z < depth; z++) {
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        vg::CopyPixel(dst, src, tex->internal_format, tex->component_type,
                      format, type);
        src += src_pixel_size;
        dst += dst_pixel_size;
      }
...
void vgTexSubImage3D(VGenum target, VGint level, VGint xoffset, VGint yoffset,
  ...
  if (xoffset < 0 || yoffset < 0 || zoffset < 0 ||
      xoffset + width > tex_level.width ||
      yoffset + height > tex_level.height ||
      zoffset + depth > tex_level.depth) {
    vg.state_.error_state = VG_INVALID_VALUE;
    return;
  }
  ...
  for (int z = 0; z < depth; z++) {
    uint8_t* z_dst =
        tex_level.memory->data() +
        ((zoffset + z) * tex_level.width * tex_level.height) * dst_pixel_size;
    for (int y = 0; y < height; y++) {
      uint8_t* x_dst =
          z_dst + ((yoffset + y) * tex_level.width + xoffset) * dst_pixel_size;
      for (int x = 0; x < width; x++) {
        vg::CopyPixel(x_dst, src, tex->internal_format, tex->component_type,
                      format, type);
        src += src_pixel_size;
...
void vgBlendFuncSeparate(VGenum sfactorRGB, VGenum dfactorRGB,
  ...
  if (!valid_factor(sfactorRGB) || !valid_factor(sfactorAlpha) ||
      !valid_factor(dfactorRGB) || !valid_factor(dfactorAlpha)) {
    vg.state_.error_state = VG_INVALID_ENUM;
    return;
  }
...
void vgBindBuffer(VGenum target, VGuint buffer) {
  ...
  VirtualGPU::BufferObject*& bound =
      (target == VG_ELEMENT_ARRAY_BUFFER && vg.bound_vertex_array_)
  ...
  if (it == vg.buffer_pool_.end()) {
    auto [new_it, _] =
        vg.buffer_pool_.emplace(buffer, VirtualGPU::BufferObject{});
    it = new_it;
  }
...
void vgBufferSubData(VGenum target, VGintptr offset, VGsizeiptr size,
  ...
  if (size > 0) {
    std::memcpy(buffer->memory->data() + offset, data,
...
void vgBlendEquationSeparate(VGenum modeRGB, VGenum modeAlpha) {
  ...
  if ((modeRGB < VG_FUNC_ADD && modeRGB > VG_MAX) ||
      modeAlpha < VG_FUNC_ADD && modeAlpha > VG_MAX) {
    vg.state_.error_state = VG_INVALID_ENUM;
    return;
  }
...
VGint vgGetUniformLocation(VGuint program, VGint name_hash) {
  ...
  prog.uniform_name_hash_to_location.insert({name_hash, location});
  return location;
}
...
void vgLinkProgram(VGuint program) {
  ...
  if (prog.is_deleted || prog.vertex_shader == nullptr ||
      prog.fragment_shader == nullptr) {
    prog.link_status = VG_FALSE;
    return;
  }
...
template <typename T>
void vgUniform1t(VGint location, T v0) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return;
  }
  ...
  VirtualGPU::Uniform& u = vg.using_program_->uniforms[it->second];
  u.data.resize(sizeof(T));
  std::memcpy(u.data.data(), &v0, sizeof(T));
}

template <typename T>
void vgUniform2t(VGint location, T v0, T v1) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return;
  }
...
template <typename T>
void vgUniform3t(VGint location, T v0, T v1, T v2) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return;
  }
...
template <typename T>
void vgUniform4t(VGint location, T v0, T v1, T v2, T v3) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return;
  }
...
template <typename T>
void vgUniform1tv(VGint location, VGsizei count, const T* value) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return;
  }
...
void vgUniformNtv(VGint location, VGsizei count, const T* value) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return;
  }
...
void vgUniform1f(VGint location, VGfloat v0) { vgUniform1t(location, v0); }
void vgUniform2f(VGint location, VGfloat v0, VGfloat v1) {
  vgUniform2t(location, v0, v1);
}
void vgUniform3f(VGint location, VGfloat v0, VGfloat v1, VGfloat v2) {
  vgUniform3t(location, v0, v1, v2);
}
void vgUniform4f(VGint location, VGfloat v0, VGfloat v1, VGfloat v2,
                 VGfloat v3) {
  vgUniform4t(location, v0, v1, v2, v3);
}
void vgUniform1i(VGint location, VGint v0) { vgUniform1t(location, v0); }
...
void vgUniformMatrix4fv(VGint location, VGsizei count, VGboolean transpose,
  ...
  if (u.type != VG_FLOAT_MAT4 || u.size != 16 || u.count < count) {
    vg.state_.error_state = VG_INVALID_OPERATION;
    return;
  }
...
template <typename T>
ALWAYS_INLINE void WriteAttrib4(VirtualGPU::ConstantAttribute& a, T x, T y, T z,
                                T w) {
...
template <typename T>
VirtualGPU::ConstantAttribute* vgVertexAttrib4t(VGuint index, T x, T y, T z,
                                                T w) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return nullptr;
...
template <typename T>
VirtualGPU::ConstantAttribute* vgVertexAttrib4Nt(VGuint index, T x, T y, T z,
                                                 T w) {
  VirtualGPU& vg = VirtualGPU::GetInstance();
  if (vg.state_.error_state != VG_NO_ERROR) {
    return nullptr;
...
void vgVertexAttrib4f(VGuint index, VGfloat x, VGfloat y, VGfloat z,
                      VGfloat w) {
  VirtualGPU::ConstantAttribute* attrib = vgVertexAttrib4t(index, x, y, z, w);
  if (attrib != nullptr) {
    attrib->type = VG_FLOAT;
...
void vgVertexAttrib4s(VGuint index, VGshort x, VGshort y, VGshort z,
                      VGshort w) {
  VirtualGPU::ConstantAttribute* attrib = vgVertexAttrib4t(index, x, y, z, w);
  if (attrib != nullptr) {
    attrib->type = VG_SHORT;
...
void vgBindBufferRange(VGenum target, VGuint index, VGuint buffer,
  ...
  if (target != VG_UNIFORM_BUFFER && target != VG_TRANSFORM_FEEDBACK_BUFFER) {
    vg.state_.error_state = VG_INVALID_ENUM;
    return;
  }

  auto& bindings = (target == VG_UNIFORM_BUFFER)
                       ? vg.uniform_buffer_bindings_
                       : vg.transform_feedback_buffer_bindings_;
...
void vgBindBufferBase(VGenum target, VGuint index, VGuint buffer) {
  ...
  vgBindBufferRange(target, index, buffer, 0,
                    static_cast<VGsizeiptr>(it->second.memory->size()));
}
...
void vgUniform1ui(VGint location, VGuint v0) { vgUniform1t(location, v0); }
...
void vgClearBufferiv(VGenum buffer, VGint drawbuffer, const VGint* value) {
  ...
  switch (buffer) {
    case VG_COLOR: {
      ...
      Color128 clear_color((float)value[0], (float)value[1], (float)value[2],
                           (float)value[3]);
    ...
    case VG_STENCIL: {
      ...
      vg.ClearDepthStencilAttachment(false, true, 0.0f,
...
void vgClearBufferfi(VGenum buffer, VGint drawbuffer, VGfloat depth,
  ...
  vg.ClearDepthStencilAttachment(true, true, depth,
...
void vgGenRenderbuffers(VGsizei n, VGuint* renderbuffers) {
  ...
  for (int i = 0; i < n; i++) {
    vg.base_handle_++;
    vg.render_buffer_pool_.insert(
        {vg.base_handle_, VirtualGPU::RenderBuffer()});
...
void vgRenderbufferStorage(VGenum target, VGenum internalformat, VGsizei width,
  ...
  if (vg.bound_render_buffer_ == nullptr ||
      vg.bound_render_buffer_->is_deleted) {
    vg.state_.error_state = VG_INVALID_OPERATION;
    return;
  }
...
VGenum vgCheckFramebufferStatus(VGenum target) {
  ...
  for (size_t slot = 0; slot < fb->draw_slot_to_color_attachment.size();
       slot++) {
...
void vgFramebufferTexture1D(VGenum target, VGenum attachment, VGenum textarget,
  ...
  const bool is_color = (attachment >= VG_COLOR_ATTACHMENT0 &&
                         attachment <= VG_COLOR_ATTACHMENT31);
  const bool is_depth = (attachment == VG_DEPTH_ATTACHMENT ||
                         attachment == VG_DEPTH_STENCIL_ATTACHMENT);
...
void vgFramebufferTexture2D(VGenum target, VGenum attachment, VGenum textarget,
  ...
  const bool is_color =
      attachment >= VG_COLOR_ATTACHMENT0 && attachment <= VG_COLOR_ATTACHMENT31;
  const bool is_depth = attachment == VG_DEPTH_ATTACHMENT ||
                        attachment == VG_DEPTH_STENCIL_ATTACHMENT;
...
void vgFramebufferTexture(VGenum target, VGenum attachment, VGuint texture,
  ...
  if (texture == 0) {
    ...
    VirtualGPU::Attachment* attch =
        (attachment == VG_DEPTH_ATTACHMENT ||
         attachment == VG_DEPTH_STENCIL_ATTACHMENT)
  ...
  switch (tex.texture_type) {
    ...
    case VG_TEXTURE_2D:
      ...
      return;
    case VG_TEXTURE_3D:
      vgFramebufferTexture3D(target, attachment, VG_TEXTURE_3D, texture, level,
                             0);
      return;
    default:
      vg.state_.error_state = VG_INVALID_OPERATION;
...
void vgSamplerParameteri(VGuint sampler, VGenum pname, VGint param) {
  ...
  switch (pname) {
    case VG_TEXTURE_WRAP_S:
      if (param == VG_CLAMP_TO_EDGE || param == VG_CLAMP_TO_BORDER ||
          param == VG_MIRRORED_REPEAT || param == VG_REPEAT)
        ds.wrap_s = param;
      else
        vg.state_.error_state = VG_INVALID_ENUM;
      break;
    ...
    case VG_TEXTURE_MIN_FILTER:
      if (param >= VG_NEAREST && param <= VG_LINEAR_MIPMAP_LINEAR)
        ds.min_filter = param;
      else
        vg.state_.error_state = VG_INVALID_ENUM;
      break;
    ...
    case VG_TEXTURE_COMPARE_MODE:
      if (param == VG_COMPARE_REF_TO_TEXTURE || param == VG_NONE)
        ds.compare_mode = param;
      else
        vg.state_.error_state = VG_INVALID_ENUM;
      break;
    ...
    case VG_TEXTURE_COMPARE_FUNC:
      if (param >= VG_NEVER && param <= VG_ALWAYS)
        ds.compare_func = param;
      else
        vg.state_.error_state = VG_INVALID_ENUM;
      break;
    ...
    case VG_TEXTURE_SWIZZLE_R:
      if ((param >= VG_RED && param <= VG_ALPHA) || param == VG_ONE ||
          param == VG_ZERO)
        ds.swizzle_r = param;
      else
        vg.state_.error_state = VG_INVALID_ENUM;
